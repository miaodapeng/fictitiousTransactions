...
<div class="pages_container">
	<div class="pages">
		...
		<ul class="pages_num">
			...
			<c:choose>
				<c:when test="${page.totalPage <=3+2}">
						...
						<c:when test="${page.pageNo<6}">
							<c:choose>
								<c:when test="${page.pageNo >3}">
									<c:forEach var="pageCount" begin="${page.pageNo-(page.totalPage-page.pageNo==0?4:(page.totalPage-page.pageNo==1?3:2))}" end="${(page.totalPage-page.pageNo)>2?(page.pageNo+2):page.totalPage}" step="1">
										<c:choose>
											<c:when test="${page.pageNo != pageCount}">
												<a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=${pageCount}&pageSize=${page.pageSize}">
													<li>${pageCount}</li>
								...
								<c:otherwise>
									<c:forEach var="pageCount" begin="1" end="5" step="1">
										<c:choose>
											<c:when test="${page.pageNo != pageCount}">
												<a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=${pageCount}&pageSize=${page.pageSize}">
													<li>${pageCount}</li>
						...
						<c:when test="${page.pageNo>=6}">
							...
							<c:choose>
								<c:when test="${page.pageNo+2 < page.totalPage}">
									<c:forEach var="pageCount" begin="${page.pageNo-2 }" end="${page.pageNo+2}" step="1">
										<c:choose>
											<c:when test="${page.pageNo != pageCount}">
												<a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=${pageCount}&pageSize=${page.pageSize}">
													<li>${pageCount}</li>
								...
								<c:otherwise>
									<c:forEach var="pageCount" begin="${page.pageNo-(page.totalPage-page.pageNo==0?4:(page.totalPage-page.pageNo==1?3:2)) }" end="${page.totalPage}" step="1">
										<c:choose>
											<c:when test="${page.pageNo != pageCount}">
												<a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=${pageCount}&pageSize=${page.pageSize}">
													<li>${pageCount}</li>
			...
			<c:choose>
				<c:when test="${page.pageNo==page.totalPage || page.totalPage == 1 || page.totalPage == 0}">
					<div class="li_wid lastpage canotclick">尾页</div>
				</c:when>
				<c:otherwise>
		...
		<c:if test="${optpage ne 'n'}"><!-- optpage等于n时不显示 -->
			<div class="chose-more-pages">
				<div class="fixed-pages">
                    <span>${page.pageSize}页</span>
                    <i class="down"></i>
                </div>
				<ul class="show-more-pages">
					<c:if test="${page.pageSize != 10}">
						<li>
							<a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=1&pageSize=10">10/页</a>
						</li>
					</c:if>
					<c:if test="${page.pageSize != 20}">
						<li>
							<a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=1&pageSize=20">20/页</a>
						</li>
					</c:if>
					<c:if test="${page.pageSize != 50}">
						<li>
							<a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=1&pageSize=50">50/页</a>
						</li>
					</c:if>
					<c:if test="${page.pageSize != 100}">
						<li>
							<%-- <a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=${page.totalPage}&pageSize=100">100/页</a> --%>
							<a href="${page.searchUrl}${fn:contains(page.searchUrl,'.do?')==true?'&':'?'}pageNo=1&pageSize=100">100/页</a>
						</li>
					</c:if>
                </ul>
			</div>
