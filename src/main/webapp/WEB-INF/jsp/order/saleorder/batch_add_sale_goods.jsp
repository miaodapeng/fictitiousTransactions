...
<c:set var="title" value="批量上传销售商品" scope="application" />
...
<script type="text/javascript">
    ...
    function subForm(){
		...
		if(parent.$("#quotePayMoneForm").find("#saleCustomerNature").val() == "465"){//客户为分销
			...
			$("#batchAddSaleGoods").find("#salesAreaId").val((zone=="0"?(city=="0"?province:city):zone));
			
			$("#batchAddSaleGoods").find("#terminalTraderName").val(terminalTraderName);
			$("#batchAddSaleGoods").find("#terminalTraderId").val(terminalTraderId);
			$("#batchAddSaleGoods").find("#terminalTraderType").val(terminalTraderType);
			$("#batchAddSaleGoods").find("#salesArea").val(salesArea);
		}
		
		$("#batchAddSaleGoods").ajaxSubmit({
			async : false,
			url : './saveBatchAddSaleGoods.do',
			data : $("#batchAddSaleGoods").serialize(),
			type : "POST",
			dataType : "json",
...
<div class="form-list  form-tips4 ml7">
	<form id="batchAddSaleGoods" method="post" enctype="multipart/form-data">
			...
			<li style="margin-bottom:8px;">
				...
				<div class='f_left'>
					...
					<div class="pop-friend-tips" style="margin: 7px 0 0 0;">
						如果您没有标准模板，请<a href="<%= basePath %>static/template/销售单批量新增产品.xlsx">下载标准模板</a>
					</div>
				</div>
			</li>
		</ul>
