...
<c:if test="${saleorder.orderType eq 3}">
	<c:set var="title" value="订货详情" scope="application" />
</c:if>
<c:if test="${saleorder.orderType eq 0 or saleorder.orderType eq 4 or saleorder.orderType eq 1 or aleorder.orderType eq 5}">
...
<script type="text/javascript">
	...
	$.ajax({
		async : false,
		url : "/order/quote/addQuoteConsultSave.do",
	...
	<div class="content mt10 ">
            ...
            <table class="table table-bordered table-striped table-condensed table-centered">
                <tbody>
					...
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${saleorder.isPrintout ne 0 && saleorder.isPrintout ne -1}">是</c:when>
                    ...
                    <tr>
                    <td>订单关闭原因</td>
                    <td colspan="3">${saleorder.closeComments }</td>
                    </tr>
                    </c:if>
                </tbody>
            </table>
        ...
        <div class="parts">
            ...
            <table class="table table-bordered table-striped table-condensed table-centered">
                <tbody>
                <c:if test="${saleorder.orderType==1}">
					<c:if test="${saleorder.traderId==null || saleorder.traderId eq 0}">
						<tr>
							<td class="table-smaller">客户名称</td>
							<td>${saleorder.traderName}</td>
							<td class="table-smaller">注册账号</td>
							<td>${saleorder.traderContactMobile}</td>
						</tr>
					</c:if>
					<c:if test="${saleorder.traderId!=null && saleorder.traderId != 0}">
					 <tr>
						<td class="table-smaller">客户名称</td>
						<td>
        ...
        <div class="parts">
            ...
            <table class="table table-bordered table-striped table-condensed table-centered">
                <tbody>
            <c:if test="${saleorder.deliveryDirect!=3}">
                    <tr>
                ...
                <tbody>
                	...
                	<c:forEach var="list" items="${saleorderGoodsList}" varStatus="staut">
	                    ...
	                    <tr <c:if test="${list.isDelete eq 1 or goodsCategoryUser eq 'n'}">class="caozuo-grey"</c:if>>
	                            ...
	                            <div class="customername pos_rel">
	                                ...
	                                <c:choose>
										...
										<c:otherwise>
											...
											<%--包装清单：${newSkuInfosMap[list.sku].PACKING_LIST}<br> <br>--%>
                            ...
                            <!- 总额计算方式变更 2018-12-27 产品Ada 总额 = 单价*(订单商品数量-订单商品售后数量)->
	                        <td><fmt:formatNumber type="number" value="${list.price * (list.num - list.afterReturnNum)}" pattern="0.00" maxFractionDigits="2" /></td>
                    ...
                    <tr style="background: #eaf2fd;">
                        ...
                        <c:otherwise>
                            ...
                            <span class="font-red"><fmt:formatNumber type="number" value="${saleorder.totalAmount}" pattern="0.00" maxFractionDigits="2" /></span>&nbsp;&nbsp;,订单实际金额&nbsp;&nbsp;
                            <span class="font-red"><fmt:formatNumber type="number" value="${realAmount}" pattern="0.00" maxFractionDigits="2" /></span>
	...
	<c:if test="${saleorder.orderType==1 }">
		<div class="parts content1">
			<div class="title-container">
				<div class="table-title nobor">报价咨询</div>
			</div>
			<table
...
5、其他规则与产品部Jason确认。"></textarea>
								<c:if test="${saleorder.status==0 or saleorder.status==4}">
									<!-- 2:失单 -->
									<c:if test="${saleorder.traderId!=null }">
										<c:if test="${roleStatus==1 }">
										<button type="button" class="font-blue mr4"
											onclick="sendQuoteConsult('${saleorder.saleorderNo}',${saleorder.quoteorderId},${num})">发送咨询</button>
										</c:if>
										<c:if test="${roleStatus!=1 }">
										<button type="button" class="font-blue mr4"
											onclick="sendQuoteConsult('${saleorder.saleorderNo}',${saleorder.quoteorderId},${num});">答复</button>
										</c:if>
									</c:if>
									<c:if test="${saleorder.traderId==null }">
										<a class="font-blue mr4 ">尚未关联ERP客户，无法发送咨询</a>
									</c:if>
									<c:set var="shenhe" value="0"></c:set>
									<c:forEach items="${verifyUserList}" var="verifyUsernameInfo">
	...
	<div class="parts content1">
      ...
      <c:if test="${saleorder.status==4 and saleorder.orderType==1 }">
      <c:if test="${curr_user.userId == saleorder.userId or curr_user.userId== 1 or curr_user.userId== 2}">
     	<button type="button" class="bt-bg-style bg-light-orange bt-small mr10" onclick="returnBDStatus(${saleorder.saleorderId})">撤回订单</button>
     	</c:if>
      </c:if>
        	...
        	<c:choose>
				<c:when test="${saleorder.validStatus eq 0}">
					<c:if test="${saleorder.traderId!=null && saleorder.traderId != 0}">			
						...
						<button type="button" class="bt-bg-style bg-light-orange bt-small mr10 addtitle" tabTitle='{"num":"order_saleorder_edit<%=System.currentTimeMillis() + (int)(Math.random()*10000) %>","link":"order/saleorder/edit.do?saleorderId=${saleorder.saleorderId}","title":"编辑订单"}'>编辑订单</button>
						</c:if>	
					</c:if>
						<c:if test="${saleorder.traderId==null || saleorder.traderId eq 0 }">
							<button type="button" class="bt-bg-style bg-light-green bt-small mr10"  onclick="alert('请先关联erp客户')">申请审核</button>
							<button type="button" class="bt-bg-style bg-light-orange bt-small mr10 " onclick="alert('请先关联erp客户')">编辑订单</button>
						</c:if>			
					<c:if test="${(null!=taskInfo and null!=taskInfo.getProcessInstanceId() and null!=taskInfo.assignee) or !empty candidateUserMap[taskInfo.id]}">
						<c:set var="shenhe" value="0"></c:set>
						<c:forEach items="${verifyUserList}" var="verifyUsernameInfo">
				...
				<c:otherwise>
					...
					<c:if test="${saleorder.lockedStatus eq 0}">
						...
						<c:if test="${invoiceModifyflag !=null && invoiceModifyflag eq 1 && saleorder.lockedStatus == 0}" >
							<button type="button" class="bt-bg-style bg-light-orange bt-small mr10" onclick="applyToModifyHcSaleorderForTrader(${saleorder.saleorderId}, '<%= basePath %>')">申请修改</button>
						</c:if>
					</c:if>
				</c:otherwise>
                ...
                <tbody>
                	...
                	<c:forEach var="list" items="${saleInvoiceApplyList}" varStatus="num">
                		...
                		<tr>
							...
							<!-- add by Tomcat.Hui 2019/11/21 11:48 .Desc: VDERP-1325 分批开票 查看已申请发票详情信息 start. -->
							<td>
								<button type="button" class="bt-bg-style bg-light-orange bt-small  pop-new-data mr10" layerParams='{"width":"80%","height":"600px","title":"查看开票申请","link":"../../finance/invoice/selectInvoiceItems.do?relatedId=${saleorder.saleorderId}&invoiceApplyId=${list.invoiceApplyId}&editFlag=false"}' >查看开票申请</button>
							</td>
							<!-- add by Tomcat.Hui 2019/11/21 11:48 .Desc: VDERP-1325 分批开票 查看已申请发票详情信息 start. -->
	                    </tr>
                	</c:forEach>
                          ...
                          <div class="print-record">
								<form method="post" id="searchSh" action="<%= basePath %>warehouse/warehousesout/printShOutOrder.do">
									 ...
									 <c:if test="${express.business_Type == 496}">
									   <span class="bt-border-style border-blue" onclick="sendMeinianExpress('${express.expressId}','${express.logisticsNo}');" >供货单同步</span>
