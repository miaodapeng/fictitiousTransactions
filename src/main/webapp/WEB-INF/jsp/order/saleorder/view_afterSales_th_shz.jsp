                	...
                	<c:if test="${(null!=taskInfo and null!=taskInfo.getProcessInstanceId() and null!=taskInfo.assignee) or !empty candidateUserMap[taskInfo.id]}">
						<c:choose>
							...
							<c:if test="${afterSalesVo.refund eq 1 || afterSalesVo.refund eq 2 || afterSalesVo.realRefundAmount ge 0}">
								<button type="button" class="bt-bg-style bg-light-green bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfo.id}&pass=true&type=2&saleorderId=${afterSalesVo.orderId}&afterSaleorderId=${afterSalesVo.afterSalesId}"}'>审核通过</button>
							</c:if>
							<c:if test="${sessionScope.curr_user.positType eq 310}">
								<button type="button" class="bt-bg-style bg-light-green bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfo.id}&pass=true&type=2&saleorderId=${afterSalesVo.orderId}&afterSaleorderId=${afterSalesVo.afterSalesId}"}'>审核通过</button>
							</c:if>
							<button type="button" class="bt-bg-style bg-light-orange bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfo.id}&pass=false&type=2&saleorderId=${afterSalesVo.orderId}&afterSaleorderId=${afterSalesVo.afterSalesId}"}'>审核不通过</button>
							</c:when>
							<c:otherwise>
