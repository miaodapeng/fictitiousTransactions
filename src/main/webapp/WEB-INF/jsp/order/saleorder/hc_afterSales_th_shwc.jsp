...
<div class="main-container">
    ...
    <div class="parts">
        <div class="title-container">
            ...
            <c:if test="${afterSalesVo.atferSalesStatus ne 3 and afterSalesVo.realRefundAmount gt 0 && afterSalesVo.refundAmountStatus ne 4 && afterSalesVo.refundAmountStatus ne 3}">
    ...
    <div class="parts">
        ...
        <table class="table  table-style6">
            ...
            <c:if test="${not empty afterSalesVo.afterSalesGoodsList}">
                ...
                <c:set var="nowSalesNum" value="0"></c:set>
                <c:set var="arrivalNum" value="0"></c:set>
                <c:set var="receiveNum" value="0"></c:set>
                <c:set var="incntNum" value="0"></c:set>
                <c:set var="directNum" value="0"></c:set>
                <c:forEach items="${afterSalesVo.afterSalesGoodsList}" var="asg" varStatus="sttaus">
                    <c:set var="sum" value="${sum+asg.num}"></c:set>
                    <c:set var="arrivalNum" value="${arrivalNum+asg.arrivalNum}"></c:set>
                    <c:set var="receiveNum" value="${receiveNum+asg.receiveNum}"></c:set>
                    <tr>
                        <td>${sttaus.count }</td>
                        <td class="text-left">
                            <div class="customername pos_rel">
                ...
                <tr>
                    <td colspan="17" class="allchosetr text-left">
        ...
        <div class="table-buttons">
            <c:if test="${afterSalesVo.atferSalesStatus ne 3 && afterSalesVo.atferSalesStatus ne 2}">
                <form action="" method="post" id="myform">
                    ...
                    <c:if test="${sum eq incntNum + systemInterceptNum && afterSalesVo.refundAmountStatus ne 4 && afterSalesVo.refundAmountStatus ne 3 && ((afterSalesVo.preAnnealing gt 0 && afterSalesVo.refund ne 0) || (afterSalesVo.payPeriodAmount gt 0 && afterSalesVo.refund eq 0))}">
                    ...
                    <c:if test="${(null==taskInfoOver and null==taskInfoOver.getProcessInstanceId())or (null!=taskInfoOver and taskInfoOver.assignee==null and empty candidateUserMapOver[taskInfoOver.id])}">
                        <c:if test="${afterSalesVo.refundAmountStatus eq 0 || afterSalesVo.refundAmountStatus eq 3 }">
                            <button type="button" class="bt-bg-style bg-light-green bt-small pop-new-data"
                                    layerParams='{"width":"780px","height":"310px","title":"选择售后原因","link":"./saleorderComplete.do?afterSalesId=${afterSalesVo.afterSalesId}&afterSalesType=756&type=${afterSalesVo.type}&orderId=${afterSalesVo.orderId }&subjectType=${afterSalesVo.subjectType }&formToken=${formToken }&traderId=${afterSalesVo.traderId }"}'>确认完成</button>
                        </c:if>
                        <c:if test="${afterSalesVo.status ne 1 && afterSalesVo.closeStatus eq 1}">
                            <button type="button" class="bt-bg-style bg-light-orange bt-small pop-new-data"
                                    layerParams='{"width":"780px","height":"310px","title":"选择售后原因","link":"./causeOfAfterSales.do?afterSalesId=${afterSalesVo.afterSalesId}&afterSalesType=756&type=${afterSalesVo.type}&orderId=${afterSalesVo.orderId }&subjectType=${afterSalesVo.subjectType }&formToken=${formToken }&traderId=${afterSalesVo.traderId }"}'>关闭订单</button>
                        </c:if>
                    </c:if>
