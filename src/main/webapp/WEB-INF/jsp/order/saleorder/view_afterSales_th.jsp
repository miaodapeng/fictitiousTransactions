                    ...
                    <tr>
                        ...
                        <td >
                            <div class="customername pos_rel">
                                ...
                                <c:choose>
                                    <c:when test="${afterSalesVo.source eq 1}">
                                        <span class="brand-color1 addtitle" style="float:none;"  tabTitle='{"num":"viewsaleorder${afterSalesVo.orderNo}","title":"订单信息",
                                        "link":"<%= basePath %>/order/hc/hcOrderDetailsPage.do?saleorderId=${afterSalesVo.orderId}"}'>${afterSalesVo.orderNo}</span>
                                    </c:when>
                                    <c:otherwise>
                                        <span class="brand-color1 addtitle" style="float:none;" tabTitle='{"num":"viewsaleorder${afterSalesVo.orderNo}","title":"订单信息",
                                        "link":"<%= basePath %>/order/saleorder/view.do?saleorderId=${afterSalesVo.orderId}"}'>${afterSalesVo.orderNo}</span>
                                    </c:otherwise>
                                </c:choose>
             	...
             	<c:if test="${afterSalesVo.atferSalesStatus ne 3 && afterSalesVo.atferSalesStatus ne 2 && (curr_user.positType eq 310 || curr_user.positType eq 312 || curr_user.positType eq 589)}">
	                <form action="" method="post" id="myform">
	             		...
	             		<c:if test="${afterSalesVo.status eq 0 || afterSalesVo.status eq 3}">
	             			<c:if test="${(null==taskInfo and null==taskInfo.processInstanceId and endStatus != '审核完成')or (null!=taskInfo and taskInfo.assignee==null)}">
                ...
                <form action="" method="post" id="myform">
                	...
                	<c:if test="${(null!=taskInfo and null!=taskInfo.getProcessInstanceId() and null!=taskInfo.assignee) or !empty candidateUserMap[taskInfo.id]}">
						<c:choose>
							...
							<button type="button" class="bt-bg-style bg-light-green bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfo.id}&pass=true&type=2&saleorderId=${afterSalesVo.orderId}"}'>审核通过</button>
	                    ...
	                    <c:forEach var="hi" items="${historicActivityInstance}" varStatus="status">
		                    <c:if test="${not empty  hi.activityName}">
			                    <tr>
			                        ...
			                        <td>
			                        	<fmt:formatDate value="${hi.endTime}" pattern="yyyy-MM-dd HH:mm:ss" />
			                        </td>
			                        <td>
				                        <c:choose>
											<c:when test="${hi.activityType == 'startEvent'}">开始</c:when>
											<c:when test="${hi.activityType == 'intermediateThrowEvent'}">结束</c:when>
											<c:otherwise>${hi.activityName}</c:otherwise>
										</c:choose>
									</td>
