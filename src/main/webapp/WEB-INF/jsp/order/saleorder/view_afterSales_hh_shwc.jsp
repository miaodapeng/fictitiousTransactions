            ...
            <div class="table-buttons">
            	<c:if test="${afterSalesVo.atferSalesStatus ne 3 && afterSalesVo.atferSalesStatus ne 2}">
	                <form action="" method="post" id="myform">
						...
						<c:if test="${ lendoutFlag ne 1}">
		             		<button type="button" class="bt-bg-style bg-light-green bt-small pop-new-data" 
								layerParams='{"width":"780px","height":"310px","title":"选择售后原因","link":"../order/saleorderComplete.do?afterSalesId=${afterSalesVo.afterSalesId}&afterSalesType=757&type=${afterSalesVo.type}&orderId=${afterSalesVo.orderId }&subjectType=${afterSalesVo.subjectType }&formToken=${formToken }&traderId=${afterSalesVo.traderId }"}'>确认完成</button>
						</c:if>
						<c:if test="${ lendoutFlag eq 1}">
		             			<button type="button" class="bt-bg-style bg-light-orange bt-small" onclick="lendout()">确认完成</button>
		             	</c:if>
		             		<c:if test="${afterSalesVo.status ne 1 && afterSalesVo.closeStatus eq 1}">
		             			<c:if test="${ lendoutFlag eq 1}">
		             			<button type="button" class="bt-bg-style bg-light-orange bt-small" onclick="lendout()">关闭订单</button>
		             			</c:if>
		             			<c:if test="${lendoutFlag ne 1 }">
			                	<button type="button" class="bt-bg-style bg-light-orange bt-small pop-new-data" 
			                	 layerParams='{"width":"780px","height":"310px","title":"选择售后原因","link":"../order/causeOfAfterSales.do?afterSalesId=${afterSalesVo.afterSalesId}&afterSalesType=757&type=${afterSalesVo.type}&orderId=${afterSalesVo.orderId }&subjectType=${afterSalesVo.subjectType }&formToken=${formToken }&traderId=${afterSalesVo.traderId }"}'>关闭订单</button>
			                	 </c:if>
			                </c:if>
					 	</c:if>
