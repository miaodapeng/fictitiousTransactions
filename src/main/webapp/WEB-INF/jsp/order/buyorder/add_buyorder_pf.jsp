...
<script type="text/javascript" src='<%= basePath %>static/js/order/buyorder/add_buyorder_pf.js?rnd=<%=Math.random()%>'></script>
	...
	<div class="pb100 content pt10"> 
		...
		<div class="parts">
			<div class="form-list  form-tips8" style='padding:0px'>
				<div style='color:#333;padding:0 0 15px 10px;font-weight:bold;'>供应商信息</div>
		...
		<div class='parts table-style77'>
			<div class="title-container mb10">
                ...
                <div class="title-click nobor  pop-new-data" 
                			layerParams='{"width":"500px","height":"350px","title":"添加运费","link":"./addFreightPage.do?buyorderId=${buyorderVo.buyorderId}&goodsId=127063"}'>
	                   	 添加运费
	            </div>
            </div>
            ...
            <table class="table table-style7" <c:if test="${bgv.goodsId eq 127063}">id="yf"</c:if>>              
            	<thead>
                    <tr>
						...
						<th class="wid12">安调信息</th>
						<th class="wid12">内部备注</th>
						<th class="wid12">采购备注</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                    <input type="hidden" name="buyorderGoodsId" value="${bgv.buyorderGoodsId}"/>
                        ...
                        <td>
                        	<span class="oneAllMoney" alt="${bgv.buyorderGoodsId}" altGood="${bgv.goodsId}">
                        		<fmt:formatNumber type="number" value="${bgv.oneBuyorderGoodsAmount}" pattern="0.00" maxFractionDigits="2" />
                        	</span>
                        </td>
                        <td>
	                    ...
	                    <td>
                        	<textarea name="installationDispaly" placeholder="是否含安调" alt="${bgv.buyorderGoodsId}" class="wid12" onblur="changComments(this,${bgv.buyorderGoodsId});">${bgv.installation}</textarea>
                        	<input type="hidden" name="installation" value="${bgv.buyorderGoodsId}|${bgv.installation}">
                        </td>
	                     <td>
                        	<textarea name="goodsCommentsDispaly" alt="${bgv.buyorderGoodsId}" class="wid12" onblur="changComments(this,${bgv.buyorderGoodsId});">${bgv.goodsComments}</textarea>
                        	<input type="hidden" name="goodsComments" value="${bgv.buyorderGoodsId}|${bgv.goodsComments}">
                        </td>
                        <td>
                        	<textarea name="insideCommentsDispaly" alt="${bgv.buyorderGoodsId}" class="wid12" onblur="changComments(this,${bgv.buyorderGoodsId});">${bgv.insideComments}</textarea>
	                                    ...
	                                    <tr>
											...
											<c:if test="${buyorderVo.deliveryDirect eq 1}">
												<input type="hidden" name="dbBuyNum" 
													value="${bgv.buyorderGoodsId}|${saleorderGoods.saleorderGoodsId}|${saleorderGoods.num-saleorderGoods.buyNum}">
					...
					<li>
						...
						<div class="f_left">
							<div class='inputfloat'>
								<input type="text" class="input-middle" id="prepaidAmount" name="prepaidAmount" value="${buyorderVo.buyorderAmount}" readonly="readonly"/> 
							</div>
	                            ...
	                            <select class="input-middle" name="takeAddressId">
	                                ...
	                                <c:forEach var="list" items="${addressList}">
	                                	<c:if test="${(buyorderVo.takeTraderAddressId != null && buyorderVo.takeTraderAddressId != 0) && list.addressId eq buyorderVo.takeTraderAddressId }">
				                    		<option value="${list.addressId}|${list.companyName}|${list.areas}" selected="selected">
