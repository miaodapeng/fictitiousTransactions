					...
					<td><c:choose>
							<c:when test="${quote.followOrderStatus eq 1}">
								...
								<a class='addtitle' href='javascript:void(0);'
									tabtitle='{"num":"viewsaleorder${quote.saleorderId}","link":"./order/saleorder/view.do?saleorderId=${quote.saleorderId}","title":"订单信息"}'>(${quote.saleorderNo})</a>
							</c:when>
			...
			<tbody>
				...
				<c:if
					...
					<tr>
						...
						<td>到货后${customer.periodDay}天内支付 <c:if
								test="${quote.logisticsCollection eq 1}"> / 物流代收</c:if>
						</td>
					</tr>
				</c:if>
...
5、其他规则与产品部Jason确认。"></textarea>
		...
		<div  class="table-buttons">
			...
			<c:if test="${quote.followOrderStatus ne 2 and quote.verifyStatus eq 1}">
				<!-- 2:失单 -->
				...
				<%-- <input type="hidden" value="${quote.quoteorderId}" id="printId"/> --%>
				<%-- <button class="bg-light-green bt-bg-style bt-small " onclick="printBj('${quote.quoteorderId}');" >打印报价单</button> --%>
				<span class="bg-light-green bt-bg-style bt-small addtitle"
							tabTitle='{"num":"quoteorderprint_${quote.quoteorderId }","link":"./order/quote/printOrder.do?quoteorderId=${quote.quoteorderId}","title":"打印报价单"}'>打印报价单</span>
				<c:if test="${empty quote.saleorderId}">
					<button type="button" class="bt-bg-style bg-light-green bt-small" onclick="quoteorderToSaleorder(${quote.quoteorderId});">转为订单</button>
					<!-- button type="button" class="bt-bg-style bg-light-green bt-small addtitle" tabTitle='{"num":"quotetoorder","link":"order/saleorder/quoteordertosaleorder.do?quoteorderId=${quote.quoteorderId}","title":"转为订单"}'>转为订单</button-->
					<button type="button" class="bt-bg-style bt-small bg-light-orange" onclick="revokeValId();">撤销生效</button>
					<c:if
						test="${(null==taskInfoClose and null==taskInfoClose.getProcessInstanceId() )or (null!=taskInfoClose and taskInfoClose.assignee==null and empty candidateUserMapClose[taskInfoClose.id])}">
						<button type="button" class="bt-bg-style bg-light-orange bt-small mr10 pop-new-data"
							layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./closeQuote.do?quoteorderId=${quote.quoteorderId}"}'>关闭报价</button>
					</c:if>
					<!-- <button type="button" class="bt-bg-style bt-small bg-light-orange pop-new-data"
					...
					<%-- <c:if test="${(null!=taskInfoClose and null!=taskInfoClose.getProcessInstanceId() and null!=taskInfoClose.assignee) or !empty candidateUserMapClose[taskInfoClose.id]}">
						<c:choose>
							<c:when
								test="${(taskInfoClose.assignee == curr_user.username or candidateUserMapClose['belong']) and shenhe!=1}">
								<button type="button" class="bt-bg-style bg-light-green bt-small mr10 pop-new-data"
									layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfoClose.id}&quoteorderId=${quote.quoteorderId}&pass=true&type=1"}'>审核通过</button>
								<button type="button" class="bt-bg-style bg-light-orange bt-small mr10 pop-new-data"
									layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfoClose.id}&quoteorderId=${quote.quoteorderId}&pass=false&type=1"}'>审核不通过</button>
							</c:when>
							<c:otherwise>
	...
	<div class="parts">
		...
		<table
			...
			<tbody>
				...
				<c:if
			test="${null==historicActivityInstance or empty historicActivityInstance}">
			<!-- 查询无结果弹出 -->
			<tr>
				<td colspan='4'>
					暂无审核记录。
	...
	<div class="parts">
		<div class="title-container">
			<div class="table-title nobor">失单审核记录</div>
		</div>
		<table
			class="table">
		<thead>
			...
			<tbody>
				...
				<c:if test="${null!=historicActivityInstanceClose}">
					<c:forEach var="hic" items="${historicActivityInstanceClose}"
						varStatus="status">
						<c:if test="${not empty  hic.activityName}">
							<tr>
								...
								<td><fmt:formatDate value="${hic.endTime}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td><c:choose>
										<c:when test="${hic.activityType == 'startEvent'}"> 
							开始
