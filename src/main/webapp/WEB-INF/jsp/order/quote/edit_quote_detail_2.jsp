...
<c:choose>
	<c:when test="${quote.followOrderStatus eq 0}">
		<c:set var="title" value="编辑报价" scope="application" />
	</c:when>
	<c:otherwise>
...
<script type="text/javascript">
	$(function(){
		var	url = page_url + '/order/quote/getQuoteDetail.do?quoteorderId='+$("#quoteorderId").val()+'&viewType=2';
		if($(window.frameElement).attr('src').indexOf("viewType=2")<0){
...
<div class="content mt10">
		...
		<table
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${quote.verifyStatus eq 0}">审核中</c:when>
							<c:when test="${quote.verifyStatus eq 1}">审核通过</c:when>
							<c:when test="${quote.verifyStatus eq 2}">审核不通过</c:when>
	...
	<div class="parts">
		<div class="title-container">
			...
			<%-- <div class="title-click nobor  pop-new-data" layerParams='{"width":"60%","height":"70%","title":"编辑客户","link":"./editQuote.do?quoteorderId=${quote.quoteorderId}"}'>编辑</div> --%>
		</div>
		<table
			class="table table-bordered table-striped table-condensed table-centered">
			<tbody>
	...
	<div class="parts ">
		<div class="title-container">
			...
			<input type="hidden" name="quoteSource" value="${quoteSource}">
			<%-- <div class="title-click nobor  pop-new-data" id="updateTerminalDiv">
					<a href="javascript:void(0);" onclick="updateTerminal('${quote.terminalTraderName}');">编辑</a>
				</div> --%>
		</div>
		<table
			class="table table-bordered table-striped table-condensed table-centered">
	...
	<div class="parts ">
		<div class="title-container">
			...
			<%-- <div class="title-click nobor  pop-new-data" layerParams='{"width":"65%","height":"75%","title":"添加产品","link":"./addQuoteGoods.do?quoteorderId=${quote.quoteorderId}"}'>添加</div> --%>
		</div>
		<table
			class="table table-bordered table-striped table-condensed table-centered">
			<thead>
			...
			<tbody>
				...
				<c:set var="isZeroPrice" value="0"></c:set>
				<c:forEach var="list" items="${quoteGoodsList}" varStatus="staut">
					<tr <c:if test="${list.isDelete eq 1}">class="caozuo-grey"</c:if>>
						...
						<%-- <td>
								<div class="caozuo">
									<c:choose>
										<c:when test="${list.isDelete eq 0}">
											<span class="caozuo-blue pop-new-data" layerparams='{"width":"60%","height":"70%","title":"编辑产品信息","link":"./editQuoteGoodsInit.do?quoteorderGoodsId=${list.quoteorderGoodsId}"}'>编辑</span>
											<span class="caozuo-red" onclick="delQuoteGoods(${list.quoteorderGoodsId});">删除</span>
										</c:when>
										<c:otherwise>
											已删除
										</c:otherwise>
							...
							<c:if test="${list.isDelete == 0 and list.price eq '0.00'}">
								<c:set var="isZeroPrice" value="1"></c:set>
							</c:if>
					</tr>
				</c:forEach>
				<tr style="background: #eaf2fd;">
					<%-- <input id="goodsNum" value="${num}"></input> --%>
					<td colspan="13" class="text-left">总件数<span class="font-red">${num}</span>，
						总金额 <span class="font-red"> <fmt:formatNumber type="number" value="${totleMoney}" pattern="0.00" maxFractionDigits="2" />
					</span>
					</td>
				</tr>
			</tbody>
	...
	<div class="parts">
		...
		<table
			...
			<thead>
				<tr>
					<th class="table-smaller">报价有效期</th>
					<th>${quote.period}</th>
					<th class="table-smaller">发票类型</th>
					<th><c:forEach var="list" items="${invoiceTypeList}" varStatus="status">
	...
	<div class="parts content1">
		...
		<table
			...
			<thead>
				<tr>
					<th class="table-smaller">时间</th>
					<th class="table-smaller">人员</th>
					<th>内容</th>
				</tr>
			</thead>
			<tbody>
...
5、其他规则与产品部Jason确认。"></textarea>
		...
		<div class="table-buttons">
			<c:if test="${quote.followOrderStatus ne 2}">
				<!-- 未失单 -->
				<!-- 修改报价状态后跳转新标签页使用 -->
				<form action="./editQuoteValIdSave.do" id="editQuoteOperationForm">
				...
				<c:if test="${quote.validStatus eq 0}">
					<!-- <button type="button" class="bt-bg-style bg-light-green bt-small mr10" onclick="updateQuoteValId(${quote.quoteorderId});">报价生效</button> -->
					<c:if test="${((null==taskInfo and null==taskInfo.getProcessInstanceId())or (null!=taskInfo and taskInfo.assignee==null and empty candidateUserMap[taskInfo.id])) and ((null==taskInfoClose and null==taskInfoClose.getProcessInstanceId() )or (null!=taskInfoClose and taskInfoClose.assignee==null and empty candidateUserMapClose[taskInfoClose.id]))}">
						...
						<button type="button" class="bt-bg-style bg-light-green bt-small mr10" onclick="applyValidQuoteorder(${quote.quoteorderId},${taskInfo.id == null ?0: taskInfo.id},${isZeroPrice})">申请审核</button>
						<span class="bt-bg-style bg-light-orange bt-small mr10 addtitle2"
						tabtitle='{"num":"viewQuote<%=System.currentTimeMillis() + (int)(Math.random()*10000) %>","link":"./order/quote/getQuoteDetail.do?quoteorderId=${quote.quoteorderId}","title":"修改报价"}'>修改报价</span>
						<c:if test="${(null==taskInfoClose and null==taskInfoClose.getProcessInstanceId() )or (null!=taskInfoClose and taskInfoClose.assignee==null and empty candidateUserMapClose[taskInfoClose.id])}">
						<button type="button" class="bt-bg-style bg-light-orange bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./closeQuote.do?quoteorderId=${quote.quoteorderId}"}'>关闭报价</button>
						</c:if>
					</c:if>
					<c:if test="${(null!=taskInfo and null!=taskInfo.getProcessInstanceId() and null!=taskInfo.assignee) or !empty candidateUserMap[taskInfo.id]}">
						<c:choose>
							...
							<button type="button" class="bt-bg-style bg-light-green bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfo.id}&quoteorderId=${quote.quoteorderId}&pass=true"}'>审核通过</button>
							<button type="button" class="bt-bg-style bg-light-orange bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfo.id}&quoteorderId=${quote.quoteorderId}&pass=false"}'>审核不通过</button>
							</c:when>
							<c:otherwise>
					...
					<c:if test="${(null!=taskInfoClose and null!=taskInfoClose.getProcessInstanceId() and null!=taskInfoClose.assignee) or !empty candidateUserMapClose[taskInfoClose.id]}">
						<c:choose>
							<c:when test="${(taskInfoClose.assignee == curr_user.username or candidateUserMapClose['belong']) and shenhe!=1}">
							<button type="button" class="bt-bg-style bg-light-green bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfoClose.id}&quoteorderId=${quote.quoteorderId}&pass=true&type=1"}'>审核通过</button>
							<button type="button" class="bt-bg-style bg-light-orange bt-small mr10 pop-new-data" layerParams='{"width":"500px","height":"180px","title":"操作确认","link":"./complement.do?taskId=${taskInfoClose.id}&quoteorderId=${quote.quoteorderId}&pass=false&type=1"}'>审核不通过</button>
							</c:when>
							<c:otherwise>
	        				<button type="button" class="bt-bg-style bt-small bg-light-greybe mr10">已申请审核</button>
							</c:otherwise>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <c:if test="${null!=historicActivityInstanceClose}">
                    <c:forEach var="hic" items="${historicActivityInstanceClose}" varStatus="status">
                    <c:if test="${not empty  hic.activityName}">
                    <tr>
                    	<td>
                    	<c:choose>
							<c:when test="${hic.activityType == 'startEvent'}"> 
							${startUserClose}
							</c:when>
							<c:when test="${hic.activityType == 'intermediateThrowEvent'}">
							</c:when>
							<c:otherwise>
								<c:if test="${historicActivityInstanceClose.size() == status.count}">
									${verifyUsersClose}
								</c:if>
								<c:if test="${historicActivityInstanceClose.size() != status.count}">
									${hic.assignee}  
                        ...
                        <td><fmt:formatDate value="${hic.endTime}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
                        <td>
                        <c:choose>
									<c:when test="${hic.activityType == 'startEvent'}"> 
							开始
							...
							<c:otherwise>   
								 <c:if test="${hic.activityName eq '申请人'}">
										${hic.activityName}  (${reason == 1?'产品原因':'客户原因'}) 
								  </c:if>  
							 	  <c:if test="${hic.activityName ne '申请人'}">
										${hic.activityName} 
								  </c:if> 
							</c:otherwise>
						</c:choose>
						</td>
                        <td class="font-red">${commentMapClose[hic.taskId]}</td>
                    </tr>
                    </c:if>
                    </c:forEach>
                    </c:if>
                 <c:if test="${null==historicActivityInstanceClose or empty historicActivityInstanceClose}">
					<tr>
						<!-- 查询无结果弹出 -->
	...
	<!-- ----------------------------------沟通记录 ------------------------------------- -->
	<%-- 	<c:import url="./communicateList.jsp">
			<c:param name="communicateList" value="${communicateList}"/>
	...
	<div class="parts content1">
		<div class="title-container">
			...
			<!-- 未失单 -->
			<c:if test="${quote.followOrderStatus ne 2}">
				<c:if test="${empty quote.verifyStatus or (quote.verifyStatus ne 0 and quote.closeVerifyStatus ne 0)}">
		...
		<table
			...
			<thead>
				<tr>
					...
					<th style="width: 80px">录音</th>
					<th>录音内容</th>
					<th style="width: 80px">联系人</th>
					<th>联系方式</th>
					<th style="width: 80px">沟通方式</th>
					<th style="width: 80px">沟通目的</th>
					<th>沟通内容</th>
			...
			<tbody>
				...
				<c:if test="${empty communicateList}">
					<tr>
						<!-- 查询无结果弹出 -->
						<td colspan="15">查询无结果！请尝试使用其他搜索条件。</td>
					</tr>
				</c:if>
			</tbody>
		</table>
