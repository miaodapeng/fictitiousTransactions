...
<c:set var="title" value="销售换货-售后" scope="application" />
		...
		<table class="table">
			<tbody>
				...
				<tr>
					...
					<td>客户等级</td>
					<td><c:if test="${afterSalesVo.customerLevel eq 931}">S</c:if>
		...
		<table class="table  table-style6">
			<thead>
				<tr>
					...
					<th class="wid6">单位</th>
					<th class="wid6">直发</th>
					<th class="wid6">换货数量</th>
					<th class="wid6">退货方式</th>
					<th class="wid6">已退货数量</th>
					<th class="wid6">已发货数量</th>
				</tr>
			</thead>
			<tbody>
				<c:if test="${not empty afterSalesVo.afterSalesGoodsList}">
					<c:set var="back_num" value="0"></c:set> 
					<c:set var="interceptNum" value="0"></c:set><!-- 系统拦截出库数量 -->
					<c:forEach items="${afterSalesVo.afterSalesGoodsList}" var="asg" varStatus="sttaus">
						<c:if test="${asg.deliveryNum > asg.num }">
                   			<c:set var="interceptNum" value="${interceptNum-asg.deliveryNum}"></c:set>
                   		</c:if>
                   		<c:if test="${asg.deliveryNum <= asg.num }">
                   			<c:set var="interceptNum" value="${interceptNum+asg.num}"></c:set>
                   		</c:if>
						<tr class="J-skuInfo-tr" skuId="${asg.goodsId}">
							...
							<td class="text-left">
								<div class="customername pos_rel">
									<span style="float:none" class="brand-color1 addtitle JskuName" tabTitle='{"num":"viewgoods${asg.goodsId}","link":"./goods/goods/viewbaseinfo.do?goodsId=${asg.goodsId}", "title":"产品信息"}'>
										${asg.goodsName}
									</span>
									<i class="iconbluemouth"></i> 
							...
							<td style="color:red;">
								<fmt:formatNumber type="number" value="${asg.num}" pattern="0.00" maxFractionDigits="2" />
							...
							<td style="color:red;"><fmt:formatNumber type="number" value="${asg.arrivalNum}" pattern="0.00" maxFractionDigits="2" /></td>
							<td style="color:red;"><fmt:formatNumber type="number" value="${asg.deliveryNum}" pattern="0.00" maxFractionDigits="2" /></td>
						</tr>
					</c:forEach>
					<tr>
		...
		<table class="table  table-style6">
			<thead>
				<tr>
					...
					<th class="wid18">快递单号</th>
					<th class="wid12">发货时间</th>
					<th class="wid8">运费</th>
					<th class="wid8">商品</th>
					<th class="wid5">备注</th>
			...
			<tbody>
				<c:if test="${not empty afterSalesVo.expresseList}">
					<c:forEach items="${afterSalesVo.expresseList}" var="express" varStatus="num_index">
						<tr>
	                        ...
	                        <td>
	                        	<c:forEach var="user" items="${afterSalesVo.userList}">
									<c:if test="${user.userId eq express.creator}">${user.username}</c:if>
								</c:forEach>
	                        </td>
	                        <td>
