...
<c:set var="title" value="${shjsp}" scope="application" />
...
<div class="main-container">
	...
	<div class="parts">
				...
				<table class="table  table-style10">
					<thead>
						<tr>
							...
							<th class="wid20">服务时间</th>
							<th class="wid15">工程师酬金</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							...
							<td>
								<span style="color: red;">
									<fmt:formatNumber type="number" value="${asg.engineerAmount}" pattern="0.00" maxFractionDigits="2" />
								</span>
							</td>
						</tr>
						<tr>
							<td colspan="5" class="table-container">
								<table class="table">
									...
									<tbody>
										<c:choose>
											<c:when test="${not empty asg.afterSalesGoodsVoList}">
												<c:forEach items="${asg.afterSalesGoodsVoList}" var="asgv" varStatus="sttaus">
													<tr class="J-skuInfo-tr" skuId="${asgv.goodsId}">

														<td class="JskuCode"> </td>
														<td class="text-left">
															<div class="customername pos_rel">
																<span style="float:none" class="brand-color1 addtitle JskuName"
																	tabTitle='{"num":"viewgoods${asgv.goodsId}","link":"./goods/goods/viewbaseinfo.do?goodsId=${asgv.goodsId}",
														...
														<td><fmt:formatNumber type="number" value="${asgv.saleorderPrice}" pattern="0.00" maxFractionDigits="2" /></td>
														<td><fmt:formatNumber type="number" value="${asgv.saleorderNum}" pattern="0.00" maxFractionDigits="2" /></td>
														<td class="JskuUnit">${asgv.unitName}</td>
														<td>
															<span style="color: red;">
																<fmt:formatNumber type="number" value="${asgv.num}" pattern="0.00" maxFractionDigits="2" />
															</span>
														</td>
													</tr>
												</c:forEach>
		...
		<c:if test="${empty afterSalesVo.afterSalesInstallstionVoList}">
			<div class="noresult" style="margin-top:0px;">暂无记录</div>
		</c:if>
	</div>

	<div class="parts">
		<div class="title-container">
			<div class="table-title nobor">安调信息</div>
		</div>
		<table class="table">
			<tbody>
				<tr>
					<c:choose>
						<c:when test="${afterSalesVo.type eq 541}">
							<td>安调费</td>
						</c:when>
						<c:otherwise>
							<td>维修费</td>
	...
	<div class="parts">
		<div class="title-container">
			...
			<c:if test="${afterSalesVo.atferSalesStatus eq 1}"><!-- 進行中 -->
				<div class="title-click nobor  pop-new-data" layerParams='{"width":"45%","height":"55%","title":"新增交易记录",
