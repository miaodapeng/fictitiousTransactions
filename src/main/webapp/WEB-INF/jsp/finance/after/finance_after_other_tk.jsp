...
<c:set var="title" value="第三方退款" scope="application" />
		...
		<table class="table">
			<tbody>
				<tr>
					...
					<td>账户余额</td>
					<td class="warning-color1"><fmt:formatNumber type="number" value="${afterSalesVo.traderAmount}" pattern="0.00" maxFractionDigits="2" /></td>
				</tr>
				<tr>
	...
	<div class="parts">
		<div class="title-container">
			...
			<c:if test="${afterSalesVo.atferSalesStatus eq 1}"><!-- 進行中 -->
				<c:if test="${afterSalesVo.haveRefundAmount < afterSalesVo.refundAmount}"><!-- 已收金额小于实退金额 -->
				...
				<c:if test="${not empty afterSalesVo.afterCapitalBillList}">
					<c:forEach items="${afterSalesVo.afterCapitalBillList}" var="acb">
						<tr>
							...
							<td class="warning-color1"><fmt:formatNumber type="number" value="${acb.amount}" pattern="0.00" maxFractionDigits="2" /></td>
							<td>
