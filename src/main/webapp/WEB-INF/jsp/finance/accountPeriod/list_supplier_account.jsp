...
<c:set var="title" value="供应商账期记录" scope="application" />
...
<script type="text/javascript" src='<%=basePath%>static/js/finance/accountPeriod/list_customer_account.js?rnd=<%=Math.random()%>'></script>
	<div class="main-container">
		<div class="list-pages-search">
			<form method="post" id="search" action="<%=basePath%>finance/accountperiod/getSupplierAccountListPage.do">
				<ul>
					<li>
					...
					<li>
						...
						<input type="text" class="input-middle" name="buyorderNo" id="buyorderNo" value="${ap.buyorderNo}" />
					</li>
					<li>
				...
				<div class="tcenter">
					...
					<span class="bt-small bg-light-blue bt-bg-style mr20" onclick="exportSupplierAccount();">导出</span>
				</div>
			</form>
		</div>
		<div class="list-page">
				...
				<div class="superdiv">
					<table class="table table-bordered table-striped table-condensed table-centered">
						...
						<tbody>
							...
							<c:forEach var="list" items="${supplierAccountList}" varStatus="num">
								<tr>
									<td>${num.count}</td>
									<td>
										<a class="addtitle" href="javascript:void(0);" tabtitle='{"num":"financesupplier${list.traderId}", "link":"./trader/supplier/baseinfo.do?traderId=${list.traderId}", "title":"客户信息"}'>${list.traderName}</a>
									</td>
									<td>
									...
									<td>
										<c:if test="${! empty list.buyorderNo}">
											<c:set var="orderNumPage" value="${orderNumPage+1}"></c:set>
										</c:if>
										<a class="addtitle" href="javascript:void(0);" tabtitle='{"num":"viewbuyorder${list.relatedId}","link":"./order/buyorder/viewBuyorder.do?buyorderId=${list.relatedId}","title":"订单信息"}'>${list.buyorderNo}</a>
									</td>
							...
							<c:if test="${empty supplierAccountList}">
								<tr>
									<td colspan="12">
