...
<script type="text/javascript">
	$(document).ready(function()
	{
		...
		{	
			...
			{
				// 获取categoryId
				var cid = $(this).find(":first-child").val();
				var new_text = '';
				var old_text = $("#pid_"+cid).text();
				// 1级判断是否存在
    ...
    <div class="main-container">
    	<div class="list-pages-search">
			<form method="post" id="search" action="<%=basePath%>firstengage/category/index.do">
				<ul>
	                <%-- <li>
            ...
            <table class="table table-bordered table-striped table-condensed table-centered">
                <thead>
                    <tr class="sort">
                      	...
                      	<th class="wid15">分类级别</th>
                        <th class="wid15">分类核心商品数</th>
                        <th class="wid15">分类临时商品数</th>
                        <th class="wid15">分类其他商品数</th>
                        <th class="wid15">上级分类</th>
        		...
        		<c:if test="${not empty categoryList}">
                	<c:forEach items="${categoryList }" var="list" varStatus="status">
	                    <tr>
	                    	<input type="hidden" value="${list.categoryId}"></td>
	                        <td class="textindent8">
		                        <c:if test="${list.level<3}">
		                        	<a href="<%=basePath%>firstengage/category/index.do?parentId=${list.categoryId}" onclick="waitWindowNew('no');">
							...
							<td>${list.goodsNum}</td>
							<td>
								<span id="pid_${list.categoryId}"></span>
							</td>
							<td>
								
								&nbsp;&nbsp;&nbsp;
	                        	  <c:if test="${list.level<3}">
		                        	<span class="edit-user pop-new-data" layerParams='{"width":"500px","height":"180px","title":"编辑分类","link":"./addCategory.do?categoryId=${list.categoryId}"}'>
										新增下级
									</span>
		                        	<a href="<%=basePath%>firstengage/category/index.do?parentId=${list.categoryId}" onclick="waitWindowNew('no');">
		                        		<span>查看下级</span>
