...
<div class="main-container">
		...
		<table class="table">
			<tbody>
				...
				<tr>
					...
					<td class="text-left">${webAccountVo.mobile }</td>
	...
	<form method="post"  action="${pageContext.request.contextPath}/trader/accountweb/saveedit.do" id="myform">
		<div class="parts">
			<div class="title-container">
			...
			<table class="table">
				<tbody>
					<tr>
						<td class="table-smallest"><span class="warning-color1">*</span>关联客户</td>
						<td class="text-left" colspan="3">
					...
					<tr>
						<td class="table-smallest"><span class="warning-color1">*</span>联系人</td>
						<td class="text-left" colspan="3">
							<select name="traderContactId" id="traderContactId" style="width: 300px;">
								<option value="0" title="请选择">请选择</option>
								<c:if test="${not empty  contactList}">
									<c:forEach items="${contactList }" var="contact">
										<option value="${contact.traderContactId }" <c:if test="${contact.traderContactId eq webAccountVo.traderContactId}">selected="selected"</c:if> title="${contact.mobile }">${contact.name }|${contact.mobile }</option>
									</c:forEach>
								</c:if>
							</select>
							<a href="javascript:void(0);" onclick="addContacts()">添加联系人</a>
							<span class="mt4 pop-new-data font-blue" id="addCustomerContacts" display: none;></span>
						</td>
					</tr>
					<tr>
						<td class="table-smallest"><span class="warning-color1">*</span>联系地址</td>
						<td class="text-left" colspan="3">
							<select name="traderAddressId" id="traderAddressId" style="width: 500px;">
								<option value="0">请选择</option>
								<c:if test="${not empty  addressList}">
									<c:forEach items="${addressList}" var="address">
										<option value="${address.traderAddress.traderAddressId }" <c:if test="${address.traderAddress.traderAddressId eq webAccountVo.traderAddressId}">selected="selected"</c:if>>${address.area }&nbsp;${address.traderAddress.address }</option>
									</c:forEach>
								</c:if>
							</select>
							<a href="javascript:void(0);" onclick="addAddress()">添加地址</a>
							<span class="mt4 pop-new-data font-blue" id="add_address" style="display: none;"></span>
						</td>
					</tr>
				</tbody>
			</table>
		...
		<div class="add-tijiao tcenter">
			<input type="hidden" name="optType" value="saleWeb"><!--标记区分入口-->
			...
			<button type="button" class="bt-bg-style bg-light-green bt-small" onclick="saveWebAccount();">提交</button>
		</div>
	</form>
</div>
<script type="text/javascript" src="${pageContext.request.contextPath}/static/js/webaccount/view_webaccount.js?rnd=<%=Math.random()%>"></script>
