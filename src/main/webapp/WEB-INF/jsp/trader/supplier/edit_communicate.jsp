...
<div class="baseinforcontainer" style="padding-bottom: 15px;">
	...
	<div class="baseinforeditform">
		<form method="post"
			action="${pageContext.request.contextPath}/trader/supplier/saveeditcommunicate.do"
			id="form">
			<ul>
				<li>
						...
						<ul id="communicateGoalDiv">
							<c:if test="${not empty communicateGoal }">
								<c:forEach items="${communicateGoal }" var="goal">
									<c:if
										test="${goal.sysOptionDefinitionId != 268 and goal.sysOptionDefinitionId != 468}">
			...
			<div class="add-tijiao tcenter ">
				...
				<button class="dele" id="close-layer" type="button"
					onclick="goUrl('${pageContext.request.contextPath}/trader/supplier/communicaterecord.do?traderId=${communicateRecord.traderId}&traderSupplierId=${communicateRecord.traderSupplierId}');">取消</button>
			</div>
		</form>
	</div>
</div>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/static/js/supplier/add_communicate.js?rnd=<%=Math.random()%>"></script>
<script type="text/javascript"
