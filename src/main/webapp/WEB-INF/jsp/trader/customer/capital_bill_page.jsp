					...
					<c:if test="${not empty capitalBill}">
						<c:forEach items="${capitalBill}" var="acb">
							<tr>
								...
								<td>
									<c:if test="${acb.capitalBillDetail.orderType eq 1}">
										<a class="addtitle1" href="javascript:void(0);" tabTitle='{"num":"viewbuyorder<%=System.currentTimeMillis() + (int)(Math.random()*10000) %>",
											"link":"<%= basePath %>/order/saleorder/view.do?saleorderId=${acb.capitalBillDetail.relatedId}","title":"订单信息"}'>${acb.capitalBillDetail.orderNo}</a>
									</c:if>
									<c:if test="${acb.capitalBillDetail.orderType eq 3}">
										<a class="addtitle1" href="javascript:void(0);" tabTitle='{"num":"viewbuyorder<%=System.currentTimeMillis() + (int)(Math.random()*10000) %>",
											"link":"<%= basePath %>/aftersales/order/viewAfterSalesDetail.do?afterSalesId=${acb.capitalBillDetail.relatedId}&traderType=1","title":"订单信息"}'>${acb.capitalBillDetail.orderNo}</a>
									</c:if>
								</td>
