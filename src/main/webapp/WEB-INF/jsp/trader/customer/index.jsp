	...
	<div class="list-pages-search">
		<form action="${pageContext.request.contextPath}/trader/customer/index.do" method="post" id="search">
           ...
           <ul>
            ...
            <li><label class="infor_name">客户资质审核</label>
                <select class="input-middle f_left" name="aptitudeStatus">
                    <option value="">全部</option>
                    <option value="3" <c:if test="${traderCustomerVo.aptitudeStatus eq 3 }">selected="selected"</c:if>>待审核</option>
                    <option value="0" <c:if test="${traderCustomerVo.aptitudeStatus eq 0 }">selected="selected"</c:if>>审核中</option>
                    <option value="1" <c:if test="${traderCustomerVo.aptitudeStatus eq 1 }">selected="selected"</c:if>>审核通过</option>
                    <option value="2" <c:if test="${traderCustomerVo.aptitudeStatus eq 2 }">selected="selected"</c:if>>审核不通过</option>
                </select></li>
			   <li>
					...
					<c:if test="${not empty list}">
						<c:forEach items="${list}" var="traderCustomerVo"
							...
							<tr>
								...
								<td> <fmt:formatNumber type="number" value="${traderCustomerVo.buyMoney}" pattern="0.00" maxFractionDigits="2" /></td>
								...
								<td>
									...
									<c:if test="${traderCustomerVo.aptitudeStatus eq 0}">审核中</c:if>
