...
<c:set var="title" value="外借入库详情" scope="application" />
	...
	<form method="post" id="addWarehouseIn"
			...
			<table class="table">
				<tbody>
					<tr>
						...
						<td class="wid20">借用原因</td>
						<td><c:if test="${lendout.lendOutType==1 }">样品外借</c:if>
						<c:if test="${lendout.lendOutType==2 }">售后垫货</c:if></td>
					</tr>
					<tr>
					...
					<tr>
						<td class="wid20">出库数量</td>
						<td><font color="red">${goods.num}</font></td>
						<td class="wid20">单位</td>
						<td>${goods.unitName}</td>
					</tr>
					<tr>
						<td class="wid20">未归还数量</td>
						<td>${goods.num-wNum}</td>
						<td class="wid20">外借数量</td>
						<td>${lendout.lendOutNum}</td>
					</tr>
					<tr>
						<td class="wid20">已归还数量</td>
		...
		<div class="parts">
			...
			<div class="table-buttons">
			    ...
			    <input type="hidden" id="ywtype"  name="ywtype" value="4"/>
				<input type="hidden" id="bNum" value="${goods.num}"/>
				<input type="hidden" id="wNum" value="${wNum}"/>
				<input type="hidden" name="buyorderId" value="${lendout.lendOutId}"/>
				<input type="hidden" name="warehouseInNum" value="" /> <input
					type="hidden" name="buyorderGoodsId" id="buyorderGoodsId"
					value="${lendout.lendOutId}" /> <input
...
<script type="text/javascript"
	src='<%=basePath%>static/js/logistics/warehouseIn/add_lendout_WarehouseIn.js?rnd=<%=Math.random()%>'></script>
