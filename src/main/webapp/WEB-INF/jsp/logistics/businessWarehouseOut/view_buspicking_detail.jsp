	...
	<div class="main-container">
		...
		<div class="parts table-style10-1 ">
            ...
            <div class="pick-pro-list">
               <div class="f_left">合同单号：${afterSales.afterSalesNo}</div> 
			...
			<table class="table  table-style10">
								...
								<tbody>
								   <c:forEach var="listw" items="${list.wlist}" varStatus="num2">
									<tr>
										...
										<td><date:date value="${listw.expirationDate }" format="yyyy-MM-dd"/></td>
										<td><date:date value="${listw.checkStatusTime }" format="yyyy-MM-dd"/></td>
										<td>${listw.batchNumber }</td>
										<td>${listw.warehouseArea }</td>
										<td>${listw.comments }</td>
										<td>
			 ...
			 <div class="table-buttons">
                <span class=" bt-border-style border-blue bt-small" onclick="preview('jh_mdiv')">打印拣货单</span>
                 <span class="bg-light-blue bt-bg-style bt-small addtitle"
					tabTitle='{"num":"warehousesout_warehouseSmOut_${afterSales.afterSalesId }","link":"./warehouse/businessWarehouseOut/warehouseSmOut.do?afterSalesId=${afterSales.afterSalesId }&businessType=${businessType }","title":"扫码打包发货"}'>拣货完成，扫码打包发货</span>
               <%--  <span class="bg-light-blue bt-bg-style bt-small addtitle"
