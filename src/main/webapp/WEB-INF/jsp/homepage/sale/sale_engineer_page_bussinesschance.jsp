        ...
        <div class="list-page normal-list-page">
				<table
					...
					<thead>
						<tr>
							<th class="wid4">序号</th>
							<th class="wid14">商机编号</th>
							<th class="wid14">分配时间</th>
							<th class="wid10">商机状态</th>
							<th class="">客户名称</th>
							<th>询价产品</th>
							<th class="wid14">最近一次沟通时间</th>
							<th class="wid14">下次沟通时间</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="list" items="${bussinessChanceVo}" varStatus="num">
							<tr>
								<td>${num.count}</td>
								<td>
									<div class="font-blue">
										<a class="addtitle" href="javascript:void(0);"
											tabtitle='{"num":"view${list.bussinessChanceId}",
											"link":"./order/bussinesschance/toSalesDetailPage.do?bussinessChanceId=${list.bussinessChanceId}&traderId=${list.traderId}","title":"商机编号"}'>${list.bussinessChanceNo}</a>
									</div>
								</td>
								<td><date:date value="${list.assignTime}"/></td>
								<c:choose>
									<c:when test="${list.status eq 0}"><td style="color: red;">未处理</td></c:when>
									<c:when test="${list.status eq 1}"><td style="color: green;">报价中</td></c:when>
									<c:when test="${list.status eq 2}"><td style="color: green;">已报价</td></c:when>
								</c:choose>
								<td>
									<div class="font-blue">
										<a class="addtitle" href="javascript:void(0);"
											tabtitle='{"num":"view_quote_customer${list.traderId}",
											"link":"./trader/customer/baseinfo.do?traderId=${list.traderId}","title":"客户信息"}'>${list.checkTraderName}</a>
									</div>
								</td>
								<td>${list.content}</td>
								<td><date:date value="${list.starttimeLong}" /></td>
								<td>${list.nextContactDate}</td>
							</tr>
						</c:forEach>
						<c:if test="${empty bussinessChanceVo}">
							<tr>
								<td colspan="8">
...
<%--         <div class="daily-event">
            <ul>
	            ...
	            <c:if test="${empty saleEngineerDataVo || empty saleEngineerDataVo.bussinessChanceVoList }">
	            	<span style="font-size: 50px;color: #3384EF;">今日无待跟进商机</span>
	            </c:if>
            </ul>
        </div> --%>
        <!--  
