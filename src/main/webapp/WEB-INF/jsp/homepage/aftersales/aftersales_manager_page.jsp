...
<c:set var="title" value="售后主管" scope="application" />
...
<!--  
        <div class="daily-event quote-continue">
            <ul>
	            ...
	            <c:if test="${empty saleEngineerDataVo || empty saleEngineerDataVo.quoteorderVoList }">
	            	<span style="font-size: 50px;color: #3384EF;">今日无待跟进报价</span>
	            </c:if>
            </ul>
        </div>
        -->
        ...
        <!-- 2018-08-01 Barry 新样式 -->
        	...
        	<div class="list-page normal-list-page">
				<table
					...
					<thead>
						<tr>
							<th class="wid11">售后单号</th>
							<th class="wid12">售后类型</th>
							<th class="wid8">售后对象</th>
							<th class="wid10">联系人</th>
							<th class="wid15">联系方式</th>
							<th class="">本次沟通内容</th>
							<th class="wid15">上次联系时间</th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="cm" items="${crvList}" varStatus="num">
							<tr>
								<td>
									<c:if test="${not empty cm.orderNo}">
										<span class="font-blue addtitle" tabTitle='{"num":"viewaftersales<%=System.currentTimeMillis() + (int)(Math.random()*10000) %>",
											"link":"./aftersales/order/viewAfterSalesDetail.do?afterSalesId=${cm.afterSalesId}&traderType=1","title":"售后详情"}'>${cm.afterSalesNo}</span>
									</c:if>
									<c:if test="${empty cm.orderNo}">
										<span class="font-blue addtitle" tabTitle='{"num":"viewaftersales<%=System.currentTimeMillis() + (int)(Math.random()*10000) %>",
											"link":"./aftersales/order/viewAfterSalesDetail.do?afterSalesId=${cm.afterSalesId}","title":"售后详情"}'>${cm.afterSalesNo}</span>
									</c:if>
								</td>
								<td>${cm.typeName}</td>
								<td class="font-blue">
			                        <c:if test="${cm.realTraderType eq 1}">
			                        	<span class="brand-color1 addtitle" tabTitle='{"num":"viewcustomer<%=System.currentTimeMillis() + (int)(Math.random()*10000) %>","title":"客户信息",
												"link":"./trader/customer/baseinfo.do?traderId=${cm.afterTraderId}"}'>${cm.traderName}</span>
									</c:if>	
									<c:if test="${cm.realTraderType eq 2}">
										<span class="brand-color1 addtitle"  tabTitle='{"num":"viewcustomer<%=System.currentTimeMillis() + (int)(Math.random()*10000) %>","title":"供应商信息",
											"link":"./trader/supplier/baseinfo.do?traderId=${cm.afterTraderId}"}'>${cm.traderName}</span>
									</c:if>		
								</td>
								<td>${cm.name}</td>
								<td>
									<c:if test="${not empty cm.phone}">
										<span>${cm.phone}</span>
			                        	<i class="icontel cursor-pointer" title="点击拨号" onclick="callout('${cm.phone}',${cm.traderId},1,0,0,${cm.traderContactId});"></i>
			                        </c:if>
				                    
				                    <c:if test="${not empty cm.mobile}">
					                    <span>${cm.mobile}</span>
				                        <i class="icontel cursor-pointer" title="点击拨号" onclick="callout('${cm.mobile}',${cm.traderId},1,0,0,${cm.traderContactId});"></i>
			                        </c:if>
								</td>
								<td>${cm.nextContactContent}</td>
								<td><date:date value="${cm.addTime}" /></td>
							</tr>
						</c:forEach>
						<c:if test="${empty crvList}">
							<tr>
								<td colspan="7">
									 暂无数据！
								</td>
