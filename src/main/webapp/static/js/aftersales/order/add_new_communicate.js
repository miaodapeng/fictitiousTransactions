...
function changeCon(data){
	var id = $(data).attr("id");
	var traderId = $("#"+id).find("option:selected").attr("id");
	$.ajax({
			...
			if(data.code==0){
				...
				for(var i=0;i<(data.listData).length;i++){
					...
					if((data.listData)[i].mobile !='' && (data.listData)[i].mobile != null){
						str = str + "|"+(data.listData)[i].mobile;
					}else{
						str = str + "</option>"
					}
				}
