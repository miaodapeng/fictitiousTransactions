$(function(){
	$("#auditPayApplyReason :input").keydown(function (e) {
	      if (e.which == 13) {
	    	  btnSub();
	        return false;
	      }
...
function btnSub() {
	checkLogin();
	var $form = $("#auditPayApplyReason");
	var auditReason = $("#validComments").val();
	if (auditReason.length < 1 || auditReason.length > 128) {
		warn2Tips("validComments", "审核原因长度应该在1-128个字符之间");// 文本框ID和提示用语
		return false;
	}
	/*var nameReg = /^[a-zA-Z0-9\u4e00-\u9fa5\.\(\)\,]{1,128}$/;
	if (!nameReg.test(auditReason)) {
		warn2Tips("validComments", "审核原因不允许使用特殊字符");
		return false;
	}*/
	$.ajax({
		async : false,
