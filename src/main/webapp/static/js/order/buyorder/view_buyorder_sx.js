...
//	if(frameObj.src.indexOf("viewBuyordersh")<0){
//		frameObj.src = url;
//	}
		...
		success: function(data){
			...
			for(var i=0;i<expressList.length;i++){
				 ...
				 wlhtml=wlhtml+"<tr><td>"+express.logisticsNo+"</td><td>"+express.logisticsName+"</td>" +
				 		...
				 		wlhtml=wlhtml+" <div class='customername pos_rel'> " +
					 					...
					 					"logisticsNo="+express.logisticsNo +"\"}'>查看物流</i>"+
					 					"</div><div class='pos_abs customernameshow mouthControlPos'>最新信息："+express.contentNew+"</div></div>";
				 		wlhtml=wlhtml+"</td></tr>";
			}
			...
			if(logVoList!=null&&logVoList.length>0){
				...
				for(var i=0;i<logVoList.length;i++){
					...
					rkhtml=rkhtml+"<tr><td>"+(num*1+1)+"</td>" +
							...
							"<td>"+productDate+"</td>" +
							"<td>"+expirationDate+"</td><td>"+(agolv.batchNumber == null ? '' : agolv.batchNumber)+"</td>" +
