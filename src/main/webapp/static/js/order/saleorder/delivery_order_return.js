function deliveryOrderReturnSubmit (num) {
	...
	if (file_name == '') {
		layer.alert("请先选择送货单回传",{ icon: 2 });
		return;
	}
	
	$.ajax({
		url:'./deliveryOrderReturnSave.do',
		data:$('#delivery_order_return').serialize(),
		type:"POST",
		dataType : "json",
		async: false,
	...
	/*
	$("#delivery_order_return").ajaxSubmit({
		async : false,
		url : './deliveryOrderReturnSave.do',
		data : $("#delivery_order_return").serialize(),
		type : "POST",
...
function uploadFile(obj,num){
	...
	if($(obj)[0].files[0].size > 10*1024*1024){//字节
		layer.alert("图片文件大小应为10MB以内",{ icon: 2 });
		return;
	}
