...
function editRecord(obj){
	...
	//点击'编辑'按钮的同时, 隐藏'删除'按钮
	editLi.find('.record-dlt').hide();    
}



//'取消' 按钮   (点击'取消'按钮后,要重新显示'删除按钮')
function cancleEdit(obj){
	var editLi = $(obj).parents('li');
	editLi.removeClass('edit-li');
	//点击'取消'按钮后, 重新展示出'删除'按钮
	editLi.find('.record-dlt').show();
}
...
//删除记录,逻辑删除
function pagesContrlpages1111111(approvalId){
	checkLogin();
	layer.confirm("您是否确认删除？", {
