function contractReturnSubmit (num) {
	checkLogin();
	var file_name = $("#file_name_" + num).val();
	if (file_name == '') {
		layer.alert("请先选择入库附件",{ icon: 2 });
		return;
	}
...
function uploadFile(obj,num){
	...
	$("#img_del_" + num).hide();
	$.ajaxFileUpload({
		url : page_url + '/logistics/warehousein/contractReturnUpload.do', //用于文件上传的服务器端请求地址
