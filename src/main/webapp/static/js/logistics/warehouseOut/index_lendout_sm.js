$(document).ready(function(e) {
	...
	document.onkeydown = function(e){
	    ...
	    if((e.keyCode || e.which) == 13){
        	...
        	var orderId = $("#lendOutId").val();
        		//console.log(orderId);
        	
        	var bussinessType = $("#businessType").val();;
    		$.ajax({
    				...
    				if(data.code == -1){
    					...
    					if(data.data!=null){
    	    	        	...
    	    	        	for(var i=0;i<whLog.length;i++){
  					          ...
  					          "<td class='text-left'>"+
  	                          " <div >"+
  	                          "<a class='addtitle' href='javascript:void(0);' tabTitle='{'num':'viewgoods"+whLog[i].warehouseGoodsOperateLogId+"','link':'./goods/goods/viewbaseinfo.do?goodsId="+whLog[i].goodsId+"','title':'产品信息'}'>"+whLog[i].goodsName+"</a>"+
  	                          "</div>"+
  	                          "<div>"+whLog[i].sku+"</div>"+
  			                  ...
  			                  "<td><input name='outNum' id = '"+whLog[i].num+"' style='width:50px;text-align:right;' value='"+whLog[i].num+"' onblur='outNums(this)'  /></td>";
  			                  ...
  			                  "<td style='display:none;'>"+whLog[i].goodsId+"</td>"+
  			                  "</tr>";
...
function search_sm() {
	...
	var point=0;
	 $("#googsListDiv").find("input").each(function () {
		 ...
		 $('#outRecode tr ').each (function (){
			 var tdArr = $(this).children();
			 if(tdArr.eq(13).text()==goodId){
				 outNum+=parseInt(tdArr.eq(8).find('input').val());
			 }
		 })
		 console.log("num+"+num);
		 console.log("outNum+"+outNum);
		 if(num<outNum){
			 layer.alert("订货号为V"+goodId+"的商品出库数不允许大于商品需出库总数"+num+"！");
			 point = -1 ;
			 $("#searchSpan").attr("onclick","search_sm();");
			 return false;
		 }
