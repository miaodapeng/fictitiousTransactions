...
function autoWarehouse(id) {
	...
	if(id=="b_checknox"){
		 var idCnt="";
		    $.each($("input[name='"+id+"']:checked"),function(){
			var warehouseGoodsOperateLogId=$(this).siblings("input[name='warehouseGoodsOperateLogId']").val();
			var num=$(this).siblings("input[name='num']").val();
			idCnt+=warehouseGoodsOperateLogId+","+num/*+","+tdArr.eq(9).find('input').val()*/;
			idCnt+="#";
		});
	}
}