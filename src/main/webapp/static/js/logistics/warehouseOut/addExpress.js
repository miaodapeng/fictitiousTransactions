...
function addExpress(ywType,orderId){
	...
	$.each($("input[name='b_checknox']:checked"),function(){
		id_num_price += $(this).val()+"|"+$(this).parent().nextAll().find("input[name='num']").val()+"|"+$(this).parent().nextAll().find("input[name='price']").val()+"|"+$(this).parent().nextAll().find("input[name='sku']").val()+"_";
	});
	if(id_num_price == ""){
	...
	var delLogisticsNo=$("input[name='delLogisticsNo']").val();
	$.ajax({
		async : false,
