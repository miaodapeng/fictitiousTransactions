...
$.extend(true, $.validator, {
	...
	methods: {
		...
		enonly: function (value, element, param) {
		...
		teleCountryNum: function (value, element) {
			value = $.trim(value);
			return this.optional(element) || /^\d+$/.test(value);
		},
		teleAreaNum: function (value, element) {
			value = $.trim(value);
			return this.optional(element) || /^\d+$/.test(value);
		},
		telephoneNum: function (value) {
			value = $.trim(value);
			return !value || /^[\d\s\-]*$/.test(value);
		},
		singleTelNum: function (val) {
			val = $.trim(val);
			return !val || /^[a-zA-Z0-9\+\(\)\-]{1,20}$/.test(val);
		}
	}
});
