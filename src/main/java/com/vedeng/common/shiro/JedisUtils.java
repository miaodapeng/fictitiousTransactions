...
public class JedisUtils {
	...
	public static long setObjectList(String key, List<?> value, int cacheSeconds) {
		...
		try {
			...
			// result = jedis.rpush(getBytesKey(key), SerializeUtil.serializeList(value));
			if (cacheSeconds != 0) {
				jedis.expire(key, cacheSeconds);
			}
	 ...
	 *       <b>Date:</b> 2017年5月5日 下午2:21:32
	 */
	public static byte[] toBytes(List<?> list) {
		return ObjectUtils.serializeList(list);
	}
	 ...
	 * @return
	 * @Note <b>Author:</b> east <br>
	 *       <b>Date:</b> 2017年5月9日 下午7:10:14
	 */
	public static void delMenuRedisByRoleIds(List<Integer> roleIds) {
		Jedis jedis = null;
		try {
			jedis = getResource();
			Set<String> keys = jedis.keys("curr_user_menu_*");
			Iterator it = keys.iterator();
			while (it.hasNext()) {
				String key = (String) it.next();
				for (Integer roleId : roleIds) {
					if (key.contains(roleId.toString()) && jedis.exists(key)) {
						jedis.del(key);
						logger.debug("del {}", key);
					}
				}
